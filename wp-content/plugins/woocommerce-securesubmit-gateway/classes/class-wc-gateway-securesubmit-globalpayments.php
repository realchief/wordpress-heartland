...
class WC_Gateway_SecureSubmit_GlobalPayments extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        require_once 'wc-gateway-securesubmit-globalpayments/class-capture.php';
        require_once 'wc-gateway-securesubmit-globalpayments/class-connect.php';
        require_once 'wc-gateway-securesubmit-globalpayments/class-data.php';
        require_once 'wc-gateway-securesubmit-globalpayments/class-lookup.php';
        require_once 'wc-gateway-securesubmit-globalpayments/class-payment.php';
        require_once 'wc-gateway-securesubmit-globalpayments/class-refund.php';
        require_once 'wc-gateway-securesubmit-globalpayments/class-revieworder.php';

        // properties
        $this->id                   = 'securesubmit_globalpayments';
        $this->method_title         = __('GlobalPayments', 'wc_securesubmit');
        ...
        $this->accountId           = $this->getSetting('accountId');
        $this->transactionPwd       = $this->getSetting('transactionPwd');
        $this->rebatePwd       = $this->getSetting('rebatePwd');
        ...
        // class references
        self::$_instance   = $this;
        $this->capture     = new WC_Gateway_SecureSubmit_GlobalPayments_Capture($this);
        $this->connect     = new WC_Gateway_SecureSubmit_GlobalPayments_Connect($this);
        $this->data        = new WC_Gateway_SecureSubmit_GlobalPayments_Data($this);
        $this->lookup      = new WC_Gateway_SecureSubmit_GlobalPayments_Lookup($this);
        $this->payment     = new WC_Gateway_SecureSubmit_GlobalPayments_Payment($this);
        $this->refund      = new WC_Gateway_SecureSubmit_GlobalPayments_Refund($this);
        $this->reviewOrder = new WC_Gateway_SecureSubmit_GlobalPayments_ReviewOrder($this);
    }
    ...
    public function checks()
    {
        ...
        } elseif (!$this->rebatePwd) {
            $messageFmt = 'SecureSubmit error: Please enter your rebate password <a href="%s">here</a>';
    ...
    public function initFormFields()
    {
        ...
        $this->form_fields = include $path . '/etc/globalpayments-options.php';
    }

    public function admin_options()
    {
        $path = dirname(plugin_dir_path(__FILE__));
        include $path . '/templates/globalpayments-options.php';
    }
    ...
    {
        if (!in_array($handle, array('securesubmit', 'woocommerce_securesubmit', 'securesubmit_globalpayments', 'woocommerce_securesubmit_globalpayments')) || strpos($tag, 'utf-8') !== false) {
            return $tag;
        }
